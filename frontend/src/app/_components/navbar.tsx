...
import { ShoppingCart, Person, Menu, Close } from "@mui/icons-material"
...
const Navbar = () => {
    ...
    // return (
    //     // @ts-ignore
    //     <nav ref={nav} className="main-menu container">
    //         <a href="/">
    //             {data.title}
    //         </a>
    ...
    //         <ul>
    //             {data.navbar?.map((e, i) => (
    //                 <li key={i}><Link href={e.href || '#'}>{e.title}</Link></li>
    //             ))}
    //         </ul>
    ...
    //             <button className="nav-control" onClick={() => {
    //                 if (!nav) return

    //                 // @ts-ignore
    //                 nav.current.classList.toggle('opened')
