...
import {
  ClientForm,
  convertUserToClientFormValues,
  convertUserToClientFormValuesOutput,
} from "~/app/_components/client-form";
import {
  getFormField,
  getErrors,
  ClientFormValuesOutput,
  ClientFormValuesInput,
...
export default function SignUp({
  ...
  const [defaultValues, setDefaultValues] =
    useState<ClientFormValuesInput | null>(
      user ? convertUserToClientFormValues(user) : null
    );

  const [data, setData] = useState<ClientFormValuesOutput | null>(
    user && user.attributes?.city
      ? convertUserToClientFormValuesOutput(user)
      : null
  );
  ...
  const {
    ...
    isFetching,
    error,
  } = api.promocodes.getPromoCode.useQuery(
    {
      promocode,
      event_id: event.id,
    },
  ...
  const sumWithoutBonuses = selectedOption
    ? selectedOption.price
    : event.attributes.price;
  const sumWithBonuces = user?.attributes.bonuses
    ? sumWithoutBonuses - user.attributes.bonuses
    : null;

  const sumWithPromocode = promocodeData
    ? sumWithoutBonuses - promocodeData.amount
    : null;
  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        <Button variant="default" className="uppercase" disabled={isDisabled}>
          Записаться
        </Button>
      </DialogTrigger>
      <DialogContent className="w-full max-w-6xl max-h-[calc(100dvh)] overflow-y-auto rounded-2xl">
        ...
        <div className="grid gap-6 md:grid-cols-[3fr,2fr] mt-3">
              ...
              <>
                <Card className="grid gap-4 content-start">
                  ...
                  <div className="flex gap-4">
                    <UserIcon className="h-8 w-8 min-w-8" />
                    <div className="grid md:grid-cols-[1fr,1fr] gap-4">
                      <LabelGroup label="Имя">{data.first_name}</LabelGroup>
                      <LabelGroup label="Фамилия">{data.last_name}</LabelGroup>
                      <LabelGroup label="Отчество">
                        {data.second_name}
                      </LabelGroup>
                      <LabelGroup label="Телефон">{data.phone}</LabelGroup>
                      <LabelGroup label="Email">{data.email}</LabelGroup>
                      <LabelGroup label="Город">{data.city}</LabelGroup>
                      <LabelGroup label="Место работы">
                        {data.workplace}
                      </LabelGroup>
                      <LabelGroup label="Должность">
                        {positions.find((e) => e.id === data.position)
                          ?.attributes.name || "-"}
                      </LabelGroup>
                      <LabelGroup label="Специальность">
                        {specs.find((e) => e.id === data.speciality)?.attributes
                          .name || "-"}
                      </LabelGroup>
                      {data.custom_speciality && (
                        <LabelGroup label="Название специальности">
                          {data.custom_speciality}
                        </LabelGroup>
                      )}
                      {data.custom_position && (
                        <LabelGroup label="Название должности">
                          {data.custom_position}
                        </LabelGroup>
                      )}
                    </div>
            ...
            {!data && (
              <>
                <Alert className="text-lg max-h-fit h-fit">
                  <GraduationCap />
                  <AlertDescription>
                    Заполните все поля корректно, после завершения курса данные
                    заполняются в сертификат.
                  </AlertDescription>
                </Alert>
          ...
          <div className="grid gap-4">
            ...
            {!selectedOption && (
              <LabelGroup label="Стоимость">
                <span className="text-2xl font-semibold">
                  {new Intl.NumberFormat("ru-RU", {
                    style: "currency",
                    currency: "RUB",
                    maximumFractionDigits: 0,
                  }).format(event.attributes.price)}
                </span>
              </LabelGroup>
            )}
            ...
            {!useBonuses && (
              <div className="grid gap-3">
                ...
                {!promocodeData && usePromocode && (
                  <LabelGroup label="Промокод">
                    <div className="grid gap-1">
                      <div className="flex gap-2">
                        <Input
                          className={cn(
                            "py-3 h-full rounded-lg",
                  ...
                  <div className="grid p-3 border border-black rounded-xl gap-3">
                    ...
                    <div className="flex gap-2 items-center ">
                      ...
                      <span className="text-sm text-green-600 font-semibold">
                        Применен промокод: {promocodeData.promocode} на скидку{" "}
                        {promocodeData.amount} руб.
                      </span>

                      <Button
                        variant={"ghost"}
                        className=" min-w-5 py-0 px-3"
                        onClick={() => {
                          setPromocode("");
                          setData(null);
                        }}
                    ...
                    {promocodeData && (
                      <div className="flex justify-between gap-3  content-center items-center ">
            ...
            <div className="bg-amber-50 rounded-lg p-3 text-amber-700 text-sm flex gap-2 items-center">
              <CircleAlert className="h-4 w-4" /> ссылка на оплату действительна
              20 минут
            </div>
            <Button
              variant={"default"}
              ...
              size="lg"
              className="mt-auto bg-[linear-gradient(135deg,#A5C83B,#2AC5A7,#189BDA)] hover:opacity-90 transition"
            >
